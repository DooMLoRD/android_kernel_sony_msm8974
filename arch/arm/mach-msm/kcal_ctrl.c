...
static DEVICE_ATTR(kcal_reset, 0644, kgamma_reset_show, kgamma_reset_store);
static DEVICE_ATTR(kcal_line, 0644, kgamma_show, kgamma_store);
...
static int kcal_ctrl_probe(struct platform_device *pdev)
{
	...
	if(rc !=0)
		return -1;
	rc = device_create_file(&pdev->dev, &dev_attr_kcal_reset);
	if(rc !=0)
		return -1;
	rc = device_create_file(&pdev->dev, &dev_attr_kcal_line);
	if(rc !=0)
		return -1;
